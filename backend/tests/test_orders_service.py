"""Action-level tests for order creation logic that the HTTP tests don't reach:
...
async def test_hash_ip_hashes_deterministically():
    h = hash_ip("1.2.3.4")
    assert h == hash_ip("1.2.3.4") and len(h) == 64  # sha256 hex
    assert h != hash_ip("1.2.3.5")


async def test_get_order_by_key_empty_key_returns_none(_sessionmaker):
    async with _sessionmaker() as db:
        assert await OrderQuery(db).by_idempotency_key("") is None


# ---- create_order ----
async def test_create_order_custom_item_has_no_weight(_sessionmaker):
    async with _sessionmaker() as db:
        order = await CreateOrderAction(db).execute(
            _payload(), idempotency_key=None, ip="1.2.3.4"
        )
...
async def test_change_status_noop_then_transition(_sessionmaker):
    async with _sessionmaker() as db:
        order = await CreateOrderAction(db).execute(
            _payload(), idempotency_key=None, ip=None
        )
        logs_before = len(order.status_log)

        action = OrderAction(db)
        await action.change_status(order, order.status)  # same status: no-op
        assert len(order.status_log) == logs_before

        await action.change_status(order, OrderStatus.confirmed)
...
async def test_idempotency_race_returns_the_winning_row(_sessionmaker):
    ...
    async with _sessionmaker() as db1:
        first = await CreateOrderAction(db1).execute(
            _payload(), idempotency_key=key, ip=None
        )
    async with _sessionmaker() as db2:
        second = await CreateOrderAction(db2).execute(
            _payload(), idempotency_key=key, ip=None
        )
...
async def test_unexpected_integrity_error_is_reraised(_sessionmaker, monkeypatch):
    ...
    async with _sessionmaker() as db1:
        await CreateOrderAction(db1).execute(_payload(), idempotency_key=None, ip=None)
    async with _sessionmaker() as db2:
        with pytest.raises(IntegrityError):
            await CreateOrderAction(db2).execute(
                _payload(), idempotency_key=None, ip=None
            )
