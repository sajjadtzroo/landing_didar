...
// Guest checkout is allowed; a session only prefills the phone (the backend
// binds the order to the session phone anyway).
...
onMounted(async () => {
  ...
  await ensure()
  if (customer.value?.phone && !phone.value) phone.value = customer.value.phone
})
